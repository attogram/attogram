...
// 1. Rename phpliteadmin.config.sample.php to config.php and change parameters in there.
//    You can set only your custom settings in config.php. All other settings will be set to defaults.
	...
	if(!isset($_GET['table']) && !isset($_GET['confirm']) && (!isset($_GET['action']) || (isset($_GET['action']) && $_GET['action']!="table_create"))) //the absence of these fields means we are viewing the database homepage
	{
		...
		if($view=="structure") //database structure - view of all the tables
		{
			...
			if ($auth->isPasswordDefault())
			{
				...
				echo sprintf($lang['warn_passwd'],(is_readable('config.php')?'config.php':PAGE))."<br />".$lang['warn0'];
				echo "</div>";
			}
